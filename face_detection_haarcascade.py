...
#trained face data
trained_front_face_data = cv2.CascadeClassifier("harcascadealgorithm/haarcascade_frontalface_default.xml")
trained_smile_data = cv2.CascadeClassifier("harcascadealgorithm/haarcascade_smile.xml")
trained_upper_body_data = cv2.CascadeClassifier("harcascadealgorithm/haarcascade_upperbody.xml")


#img = cv2.imread("images6.jfif")
webcam = cv2.VideoCapture('videos/4.mp4')


while webcam.isOpened():
    success_frame_read, fram = webcam.read()
    frame = cv2.resize(fram,(1080,720))
    ...
    for(x,y,w,h) in front_face_coordinates:
        ...
        #detect smiles
        smile_coordinates = trained_smile_data.detectMultiScale(grey_smile_face,scaleFactor=3,minNeighbors=20)
